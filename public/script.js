...
for (var i = 0; i < prodListLen; i++) {
  ...
  var prodPrice =
    '<p class="product-price">$' + prodList[i].price.toFixed(2) + "</p></div>";
...
function manageShopping(list) {
  ...
  for (var listItem = 0; listItem < shoppingListLen; listItem++) {
    var listOp =
      '<div class="shopping-list-item"><p class="list-item-name">' +
      prodList[list[listItem]].name +
      "</p>";
    var listButtons = '<div class="list-functions"><div class="qty-change">';
    var listMinus =
      '<div class="trial-box"><i class="fas fa-minus fa-xs"></i></div>';
    var listInput = '<div class="trial-box">1</div>';
    var listPlus =
      '<div class="trial-box"><i class="fas fa-plus fa-xs"></i></div></div>';
    var listTrash = '<span class="remove">Remove</span></div></div>';

    shoppingListHtml +=
      listOp + listButtons + listMinus + listInput + listPlus + listTrash;
